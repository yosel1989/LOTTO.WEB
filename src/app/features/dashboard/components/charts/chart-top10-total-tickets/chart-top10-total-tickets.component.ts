...
@Component({
  selector: 'app-chart-top10-total-tickets',
  imports: [
    ChartModule, 
    ButtonModule, 
    LoaderComponent
],
  templateUrl: './chart-top10-total-tickets.component.html',
  styleUrl: './chart-top10-total-tickets.component.scss',
  providers: [CurrencyPipe]
})
export class ChartTop10TotalTicketsComponent implements OnInit, AfterViewInit, OnChanges {
    ...
    getChartData() {
        ...
        this.api.getTop10Sales(this.request).subscribe((response: DashboardTop10SalesResponseDto[]) => {
            ...
            this.options.plugins.title.text = `Total tickets vendidos por Local - Top 10`;
            this.options.plugins.subtitle.text = `${this.start_date} al ${this.end_date}`;
            this.data.datasets[0].data = response.map(item => item.total_tickets);
            this.data = {...this.data};
