...
import { DashboardDateRangeRequestDto, DashboardTotalSalesGroupByOriginResponseDto } from 'app/features/dashboard/models/dashboard.model';
...
export const EmptyDoughnutPlugin = {
  ...
  beforeDraw(chart: any) {
    ...
    const hasData = data.datasets.some((ds: any) => ds.data.some((val: number) => val > 0));
...
@Component({
  selector: 'app-chart-total-sales-by-origin',
  imports: [
    ChartModule, 
    ButtonModule, 
    LoaderComponent
],
  templateUrl: './chart-total-sales-by-origin.component.html',
  styleUrl: './chart-total-sales-by-origin.component.scss',
  providers: [CurrencyPipe]
})
export class ChartTotalSalesByOriginComponent implements OnInit, AfterViewInit, OnChanges {
    ...
    initChart() {
        if (isPlatformBrowser(this.platformId)) {
            ...
            this.plugins = [ChartDataLabels, EmptyDoughnutPlugin];

            this.options = {
                responsive: true,
                maintainAspectRatio: false,
                animation: {
                    animateRotate: true,   // activa la animación de rotación
                    animateScale: true,    // activa la animación de escala
                    duration: 1500,        // duración en ms
                    easing: 'easeOutBounce' // tipo de easing
                },
                transitions: {
                    active: {
                    animation: {
    ...
    getChartData() {
        this.loading = true;
        this.api.getTotalSalesGroupByOrigin(this.request).subscribe((response: DashboardTotalSalesGroupByOriginResponseDto[]) => {
            this.labelNames = [];
            response.forEach(item => {
                this.labelNames.push(item.origin! + " S/. " + item.total_sales);
            });
            this.data.labels = this.labelNames;
            this.options.plugins.title.text = `Total ventas por origen`;
